import { STORAGE_KEY, SHORTLIST, initialState, validateState, placeChampion, shift, persistState } from './core.mjs';
...
function formDialog({title,description='',label='',value='',choices=null,submit='Save',confirm=false}) {
  return new Promise(resolve=>{
    ...
    input.hidden=!!choices||confirm;input.required=!choices&&!confirm;input.value=value;select.hidden=!choices;select.replaceChildren();
    ...
    $('#form').onsubmit=e=>{e.preventDefault();const v=confirm?true:choices?select.value:input.value.trim();if(v)finish(v);else input.reportValidity();};
...
async function accountAction(account){
  ...
  const choices=[{value:'rename',label:'Rename account'},...(index>0?[{value:'earlier',label:'Move account earlier'}]:[]),...(index<state.accounts.length-1?[{value:'later',label:'Move account later'}]:[]),{value:'delete',label:'Delete account'}];
...
async function placementAction(championId,account){
  ...
  const choices=[...others.map(a=>({value:`move:${a.id}`,label:`Move to ${a.name}`})),...others.map(a=>({value:`copy:${a.id}`,label:`Copy to ${a.name}`})),...(index>0?[{value:'earlier',label:'Move earlier in this account'}]:[]),...(index<account.champions.length-1?[{value:'later',label:'Move later in this account'}]:[]),{value:'remove',label:`Remove from ${account.name}`}];
  ...
  else changeAccounts(accounts=>{const a=accounts.find(a=>a.id===account.id);a.champions=action==='remove'?a.champions.filter(c=>c!==championId):shift(a.champions,championId,action==='earlier'?-1:1);},action==='remove'?'Placement removed. Notes kept.':'Champions reordered.');
}
...
function renderBoard(){
  ...
  state.accounts.forEach((a,i)=>{
    ...
    account.append(header,grid,footer);board.append(account);
  });
}
...
$('#export').onclick=()=>{save();const blob=new Blob([JSON.stringify(state,null,2)],{type:'application/json'});const url=URL.createObjectURL(blob),a=el('a');a.href=url;a.download=`league-board-${new Date().toISOString().slice(0,10)}.json`;a.click();setTimeout(()=>URL.revokeObjectURL(url),1000);notify('Backup exported. Keep it somewhere safe.');};
